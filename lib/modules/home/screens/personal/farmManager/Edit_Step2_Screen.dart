...
class EditStep2Screen extends StatefulWidget {
  final String farmId;
  final List<Map<String, dynamic>>? existingAnswers;
  const EditStep2Screen({
    super.key,
  ...
  @override
  State<EditStep2Screen> createState() => _EditQuestionScreenState();
}

class _EditQuestionScreenState extends State<EditStep2Screen> {
  ...
  void initState() {
    ...
    fetchQuestions().then((_) {
      if (widget.existingAnswers != null) {
        for (final item in widget.existingAnswers!) {
          final answer = item['answer'] ?? {};
          final questionId = answer['questionId'];
          final type =
              questions.firstWhere((q) => q['_id'] == questionId)['type'];

          if (type == 'single-choice') {
            if ((answer['selectedOptions'] as List).isNotEmpty) {
              answers[questionId] = answer['selectedOptions'][0];
            }
          } else if (type == 'multi-choice') {
            answers[questionId] = List<String>.from(
              answer['selectedOptions'] ?? [],
            );
          } else if (type == 'text') {
            answers[questionId] = answer['otherText'] ?? '';
          } else if (type == 'upload') {
            // Lưu URL ảnh cũ nếu cần
            uploadedImageUrls[questionId] = List<String>.from(
              answer['selectedOptions'] ?? [],
            );
          }

          if ((answer['otherText'] ?? '').toString().isNotEmpty) {
            answers['${questionId}_otherText'] = answer['otherText'];
          }
        }

        setState(() {}); // Cập nhật lại UI sau khi gán giá trị
      }
    });
  }
